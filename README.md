...
| | |
|---|---|
| ESPHome | 2026.x (tested with 2026.9.0) |
...
```yaml
external_components:
  - source: github://davidcoulson/esphome-lldp@v0.1.0
    components: [lldp]
```
...
lldp:
  ...
  interface: ETH_DEF         # esp_netif if_key; only change it if you know you need to

  on_neighbor:               # fires when a neighbor appears or its data changes
    - logger.log:
        format: "Plugged into %s port %s"
        args: [x.system_name.c_str(), x.port_id.c_str()]
...
- Frames are checked for bounds and TLV order. Malformed frames are counted and
  dropped.
...
tests/test_lldp_frame.cpp  host unit test for the codec
```

Run the codec tests on your computer:

```bash
c++ -std=c++20 -fsanitize=address,undefined -I components tests/test_lldp_frame.cpp components/lldp/lldp_frame.cpp -o /tmp/t && /tmp/t
```
